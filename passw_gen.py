...
def drive():
    drive_q = str(input("Do you want to save the password to your Google Drive : ")).lower()
...
def msg_invalid_option():
    print("You have selected an invalid option! Choose a valid one!")
    menu()

def del_file():
    ques1 = str(input("Are you sure :  ")).lower()
    if ques1 in ['y', 'yes']:
        if os.path.isfile("secured_pw.txt") == True:
            os.chmod("secured_pw.txt", 0o777)
...
def pw_gen():
        ...
        copy = str(input("Do you want to copy the password to the clipboard : ")).lower()
        if copy in ['y', 'yes']:
            pyperclip.copy(snap)
            pyperclip.paste()
        ques = str(input("Do you want to save the password : ")).lower()
        if ques in ['y', 'yes']:
